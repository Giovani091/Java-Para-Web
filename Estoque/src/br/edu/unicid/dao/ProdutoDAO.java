...
public class ProdutoDAO {
	...
	public Produto procurarProduto(int codigo) throws Exception {
		...
		try {
			...
			if (rs.next()) {
				...
				produto = new Produto(ca,nome,descricao,quantidade);
			}
			return produto;
		} catch (SQLException sqle) {
			throw new Exception("Erro ao inserir dados" + sqle);
	...
	public List<Produto> todosProdutos(Connection conn) throws Exception {
		try {
			conn = this.conn;
			...
			while (rs.next()) {
				...
				String descricao = rs.getString(3);
				int quantidade = rs.getInt(4);
				list.add(new Produto(codigo, nome, descricao, quantidade));
			}	
			return list;
			} catch (SQLException sqle) {
				throw new Exception("Erro ao inserir dados" + sqle);
